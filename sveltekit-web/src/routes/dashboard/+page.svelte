<script lang="ts">
  ...
  const sessionExpiringSoon = $derived(
    data.sessionTtl !== null && data.sessionTtl < SESSION_WARN_SECONDS
  );
  const sessionDaysLeft = $derived(
    data.sessionTtl !== null ? Math.ceil(data.sessionTtl / (24 * 60 * 60)) : null
  );

  const unreadCount = $derived(data.questions.filter((q) => !q.isRead).length);
  const openBoxCount = $derived(data.boxes.filter((b) => b.isOpen).length);
</script>

<svelte:head>
  <title>ダッシュボード | Mado</title>
</svelte:head>
...
<div class="px-6 py-8 max-w-4xl mx-auto">
  ...
  <div class="mb-8">
    <h1 class="text-2xl font-bold mb-1 text-primary">
      おかえりなさい、{data.session.displayName ?? `@${data.session.handle}`}
    </h1>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
    {#each [
      { label: "質問箱", value: data.boxes.length, sub: `${openBoxCount}件 受付中`, color: "text-blue-400" },
      { label: "未読の質問", value: unreadCount, sub: `全 ${data.questions.length}件`, color: "text-emerald-400" },
      { label: "総受信数", value: data.questions.length, sub: "累計", color: "text-blue-400" },
    ] as stat}
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    ...
    <div class="rounded-xl p-6 bg-surface border border-border">
      ...
      {:else}
        <div class="flex flex-col gap-2">
          {#each data.boxes.slice(0, 3) as box}
            <a
    ...
    <div class="rounded-xl p-6 bg-surface border border-border">
      ...
      {:else}
        <div class="flex flex-col gap-2">
          {#each data.questions.slice(0, 4) as q}
            <a
